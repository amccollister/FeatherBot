...
class FunCog(commands.Cog):
	...
	async def imgur(self, ctx):
		...
		while True:
			...
			link = prefix + "".join(suffix) + ".png"
			response = requests.get(link)
			if not re.search(r"removed\.png", response.url):
				with Image.open(BytesIO(response.content)) as img:
					w = img.width
					h = img.height
					ratio = w/h
					if img.width<100 or img.height<100 or w+h<400 or ratio<0.2 or ratio>5:
						discard+=1
						continue
				await ctx.send(f"**Attempts:** {attempt}\n**Discarded:** {discard}\n{response.url[:-4]}")
				break
			attempt+=1
